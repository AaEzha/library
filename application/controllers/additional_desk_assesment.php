...
class additional_desk_assesment extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->input->post('submit')<>''){
			...
			// print_r($post);
			// die;
			$this->form_validation->set_rules('id_additional', 'ID Additional', 'required');
			...
			$this->form_validation->set_rules('amel_book', 'Amel Book', 'required');
			...
			if ($this->form_validation->run() == FALSE)
			{
				...
				if($upload_copy_basic_license['status'] == '1'){
					$post['ptl'] = $post['log1'];
					$post['pel'] = $post['log2'];
					unset($post['log1']);
					unset($post['log2']);
					...
					// print_r($post);
					// die;
					$this->db->insert('additional_desk_assesment',$post);
					$data = $this->additional_model->getById($this->uri->segment(3));
					send_email($data['nama'], $data['email'], "waiting", "Additional");
